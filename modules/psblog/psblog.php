...
//        $id_tab1 = $tab1->add(true, false);
    ...
    public function install()
    {
		...
		&& $this->registerHook('displayHome')
            ...
            Configuration::updateValue('PsBLOG_LASTEST_HEIGHT', 634);
            Configuration::updateValue('PsBLOG_LASTEST_WIDTH', 893);
...
//            $id_tab1 = $tab1->add(true, false);
    ...
    public function _prepareHook()
    {
        ...
        $authors = array();
        $image_w = Configuration::get('listing_secondary_img_width', 893);
        $image_h = Configuration::get('listing_secondary_img_height', 634);
...
//					$default_rewrite[$lang['id_lang']] = $link->getModuleLink($module, $controller, array('id'=>$id_blog, 'rewrite'=>$blog->link_rewrite), null, $lang['id_lang']);
    ...
    public function getBlogsFont($params)
    {
        ...
        $authors = array();
        $image_w = (int)$config->get('listing_leading_img_width', 893);
        $image_h = (int)$config->get('listing_leading_img_height', 634);
        foreach ($blogs as $key => &$blog) {
