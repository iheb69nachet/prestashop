...
class MigrateData
{
    ...
    public function retrieveOldData()
    {
        ...
        $texts = Db::getInstance()->executeS('SELECT i.`id_shop`, il.`id_lang`, il.`text` FROM `' . _DB_PREFIX_ . 'pstbannercmsblockinfo` i
            INNER JOIN `' . _DB_PREFIX_ . 'pstbannercmsblockinfo_lang` il ON il.`id_pstbannercmsblockinfo` = i.`id_pstbannercmsblockinfo`'
        );
    ...
    /**
     * Import the old PSTBannerCmsBlock data in the new structure
     *
     * @return bool
     */
    public function insertData()
    {
        ...
        $pstbannercmsblockinfo = new PSTBannerCmsBlock();
        $pstbannercmsblockinfo->text = reset($customTexts);
        $return &= $pstbannercmsblockinfo->add();
        ...
        if (sizeof($customTexts) > 1) {
            foreach ($customTexts as $key => $text) {
                ...
                $return &= $pstbannercmsblockinfo->save();
            }
        }
