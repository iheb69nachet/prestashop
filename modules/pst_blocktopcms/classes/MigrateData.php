...
class MigrateData
{
    ...
    public function retrieveOldData()
    {
        ...
        $texts = Db::getInstance()->executeS('SELECT i.`id_shop`, il.`id_lang`, il.`text` FROM `' . _DB_PREFIX_ . 'topcmsblockinfo` i
            INNER JOIN `' . _DB_PREFIX_ . 'topcmsblockinfo_lang` il ON il.`id_topcmsblockinfo` = i.`id_topcmsblockinfo`'
        );
    ...
    /**
     * Import the old TopCmsblock data in the new structure
     *
     * @return bool
     */
    public function insertData()
    {
        ...
        $topcmsblockinfo = new TopCmsblock();
        $topcmsblockinfo->text = reset($customTexts);
        $return &= $topcmsblockinfo->add();

        if (sizeof($customTexts) > 1) {
            foreach ($customTexts as $key => $text) {
                Shop::setContext(Shop::CONTEXT_SHOP, (int) $key);
                $topcmsblockinfo->text = $text;
                $return &= $topcmsblockinfo->save();
            }
        }
