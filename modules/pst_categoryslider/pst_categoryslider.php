...
class pst_categoryslider extends Module implements WidgetInterface
{
    protected $html = '';
    ...
    public function __construct()
    {
        ...
        $this->displayName = $this->trans('Category Product Slider', array(), 'Modules.CategorySlider');
        $this->description = $this->trans('Add product categorywise in tab.', array(), 'Modules.CategorySlider');
		
        $this->ps_versions_compliancy = array('min' => '1.7', 'max' => _PS_VERSION_);
		$this->templateFile = 'module:pst_categoryslider/views/templates/front/pst_categoryslider.tpl';
    }
	
	public function install()
    {	
		$arrayDefault = array('3','4','5','6');
        Configuration::updateValue("PST_CATE_SLIDER_LIST", serialize($arrayDefault));
		Configuration::updateValue("PST_CATE_SLIDER", 1);
		Configuration::updateValue("PST_CATE_NBR", 10);
	...
	public function installDB()
    {
        ...
        $return &= Db::getInstance()->execute('
			CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'pstcategoryslider` (
			`id_pstcategoryslider` int(11) NOT NULL AUTO_INCREMENT,
			`image` varchar(128) NOT NULL,
			`id_shop` int(10) NOT NULL,
			`name_category` varchar(128) NOT NULL,
			`id_category` int(10)  NOT NULL,		
			PRIMARY KEY (`id_pstcategoryslider`)
    ...
    {
        Configuration::deleteByName('PST_CATE_SLIDER_LIST');
		Configuration::deleteByName('PST_CATE_SLIDER');
		Configuration::deleteByName('PST_CATE_NBR');
		
		$ret = true;
        if ($drop_table) {
            $ret &=  Db::getInstance()->execute('DROP TABLE IF EXISTS `'._DB_PREFIX_.'pstcategoryslider`');
        }
    ...
    {
        $output = '';
        if (((bool)Tools::isSubmit('submitPstcategoryslider')) == true) {
            $ids = Tools::getValue('PST_CATE_SLIDER_LIST');
            $nbr = Tools::getValue('PST_CATE_NBR');
            ...
            {
				Configuration::updateValue('PST_CATE_SLIDER', (int)(Tools::getValue('PST_CATE_SLIDER')));
                Configuration::updateValue('PST_CATE_NBR', (int)($nbr));
	            Configuration::updateValue('PST_CATE_SLIDER_LIST', serialize($ids));
        ...
        if (Tools::isSubmit('deletepst_categoryslider') && Tools::getValue('id_category')) {
    ...
    protected function renderForm()
    {
        ...
        $var = $values['PST_CATE_SLIDER_LIST'];
        if (!is_array($var)) {
            $var = array(1);
        }
        ...
        $helper->submit_action = 'submitPstcategoryslider';
    ...
    {
        $result =  array(
            'PST_CATE_SLIDER_LIST' =>  Tools::getValue('PST_CATE_SLIDER_LIST', Configuration::get('PST_CATE_SLIDER_LIST')),
            'PST_CATE_SLIDER' =>  Tools::getValue('PST_CATE_SLIDER', Configuration::get('PST_CATE_SLIDER')),
			'PST_CATE_NBR' =>  Tools::getValue('PST_CATE_NBR', Configuration::get('PST_CATE_NBR')),
			'category_id' => Tools::getValue('category_id', Configuration::get('category_id')),
        );

        if(!is_array($result['PST_CATE_SLIDER_LIST']) && !empty($result['PST_CATE_SLIDER_LIST'])) {
            $result['PST_CATE_SLIDER_LIST'] = unserialize($result['PST_CATE_SLIDER_LIST']);
        }
    ...
    {
        $result = Db::getInstance()->execute('INSERT  INTO `'._DB_PREFIX_.'pstcategoryslider`(`id_category`,`name_category`,`image`,`id_shop`) VALUES ('.(int)$category_id.', \''.pSQL($name_cate).'\', \''.pSQL($filename).'\','.(int)$id_shop.')');
        return $result;
    }

    public function updatecategoryicon($category_id,$name_cate,$filename)
    {
        $result = Db::getInstance()->execute('UPDATE `'._DB_PREFIX_.'pstcategoryslider` SET `name_category` = \''.pSQL($name_cate).'\',`image` =\''.pSQL($filename).'\' WHERE `id_category` = '.(int)$category_id);
        return $result;
    }

    public static function deleteCategoryId($category_id)
    {
        $sql = 'DELETE FROM`'._DB_PREFIX_.'pstcategoryslider` WHERE `id_category` = '.(int)$category_id;
        Db::getInstance()->execute($sql);
    }
    
    public function getimage($category_id,$id_shop)
    {
        $sql = 'SELECT id_pstcategoryslider ,image,id_category,name_category FROM '._DB_PREFIX_.'pstcategoryslider WHERE id_category = '.(int)$category_id.' and id_shop ='.(int)$id_shop.'' ;
        return Db::getInstance()->executeS($sql);
    }

    public function getcategoryicon()
    {
        $sql = 'SELECT id_pstcategoryslider ,image,id_category,name_category, id_shop FROM '._DB_PREFIX_.'pstcategoryslider';
        return Db::getInstance()->executeS($sql);
    }
	
	public function hookdisplayHeader($params)
    {
        $this->context->controller->registerJavascript('modules-pstcategoryslider', 'modules/'.$this->name.'/views/js/pst_categoryslider.js', ['position' => 'top', 'priority' => 150]);
		$this->context->controller->registerStylesheet('modules-pstcategoryslider', 'modules/'.$this->name.'/views/css/pst_categoryslider.css', ['media' => 'all', 'priority' => 150]);
    }
	
    public function renderWidget($hookName = null, array $configuration = [])
    {
        if (!$this->isCached($this->templateFile, $this->getCacheId('pst_categoryslider'))) {
            $this->smarty->assign($this->getWidgetVariables($hookName, $configuration));
        }

        return $this->fetch($this->templateFile, $this->getCacheId('pst_categoryslider'));
    }
	 ...
	 public function getWidgetVariables($hookName = null, array $configuration = [])
    {
        ...
        $arrayCategory = array();
        $categoryimg = '';
        $catnb = Configuration::get('PST_CATE_NBR');
		foreach ($config_values['PST_CATE_SLIDER_LIST'] as $id){
            if($id == 1) { continue; }
			...
			if(!empty($products_for_template)) {
				$arrayCategory[] = array(
					...
					'image_url' => $this->context->link->getMediaLink(_MODULE_DIR_.'pst_categoryslider/views/img'),
				);
			}
        ...
        return array(
             'pstcategorysliderinfos' => $arrayCategory,
			 'image_url' => $this->context->link->getMediaLink(_MODULE_DIR_.'pst_categoryslider/views/img'),
			 'slider' => (int)Configuration::get('PST_CATE_SLIDER'),
        );
    }
