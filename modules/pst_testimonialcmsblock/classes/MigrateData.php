...
class MigrateData
{
     ...
     * - pst_testimonialcmsblock < v3.0.0
    ...
    public function retrieveOldData()
    {
        ...
        $texts = Db::getInstance()->executeS('SELECT i.`id_shop`, il.`id_lang`, il.`text` FROM `' . _DB_PREFIX_ . 'psttestimonialcmsblockinfo` i
            INNER JOIN `' . _DB_PREFIX_ . 'psttestimonialcmsblockinfo_lang` il ON il.`id_psttestimonialcmsblockinfo` = i.`id_psttestimonialcmsblockinfo`'
        );
    ...
    /**
     * Import the old PSTTestimonialCmsBlock data in the new structure
     *
     * @return bool
     */
    public function insertData()
    {
        ...
        $psttestimonialcmsblockinfo = new PSTTestimonialCmsBlock();
        $psttestimonialcmsblockinfo->text = reset($customTexts);
        $return &= $psttestimonialcmsblockinfo->add();
        ...
        if (sizeof($customTexts) > 1) {
            foreach ($customTexts as $key => $text) {
                ...
                $return &= $psttestimonialcmsblockinfo->save();
            }
        }
